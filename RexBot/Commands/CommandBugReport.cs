            ...
            em.Fields = new List<EmbedFieldBuilder>()
                        {
                            ...
                            new EmbedFieldBuilder()
                            {
                                IsInline = false,
                                Name = "!bugreport list",
                                Value = "Gets a link to the Trello board showing all RexBot reports."
                            },
                            new EmbedFieldBuilder()
                            {
                                IsInline = false,
                                Name = "!bugreport sheet",
                                Value = "Gets a link to the Google Sheets version of the bug list."
                            },
                            new EmbedFieldBuilder()
                            {
                                IsInline = false,
            ...
            if (parts[0].Equals("list", StringComparison.CurrentCultureIgnoreCase))
            {
                if (message.CTG() || (!(message.Channel is IGuildChannel) && message.Author.CTG()))
                {
                    ...
                    em.AddField(x =>
                                {
                                    x.Name = "Public List";
                                    x.Value = RexBotCore.Instance.Trello.PublicBoard.Board.Url;
                                });
                    em.AddField(x =>
                                {
                                    x.Name = "CTG List";
                                    x.Value = RexBotCore.Instance.Trello.CTGBoard.Board.Url;
                                });
                    await message.Channel.SendMessageAsync(message.Author.Mention + " Lists update every 10 minutes.", embed: em.Build());
                }
                else
